    ...
    <div itemscope itemtype="https://schema.org/WebContent" class="lg:pt-28 pt-14">
        <div class="container max-w-7xl m-auto px-4">
            <header class="max-w-[750px] mx-auto mb-16">
                <h2 itemprop="text" class="title-primary mb-4 " v-html="title"></h2>
            </header>
            <div class="grid lg:grid-cols-2 grid-cols-1">
                <div class="lg:mb-0 mb-7">
                    <img itemprop="image" class="rounded-[10px]" :src="img"
                        alt="our process img">
                </div>

                <div>
                    <div v-for="(item, index) in chooseFaqItems" :key="index"
                                ...
                                <ul class="text-gray">
                                    <li v-for="(answer, i) in item.answers" :key="i" class=" px-6 pb-4">
                                        <div class="flex">
                                            ...
                                            <div>
                                                <p itemprop="headline" class="text-dark-black font-medium font-jost text-lg mr-2">{{
                                                    answer.title }}</p>
                                                <p itemprop="description" class="text-dark-black text-base font-poppins">{{ answer.description
                                                    }}</p>
                                            </div>
                                        </div>
                                    </li>
...
export default {
    ...
    methods: {
        toggleItem(index) {
            this.chooseFaqItems.forEach((item, i) => {
